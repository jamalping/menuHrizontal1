...
-(void)createMenuItems:(NSArray *)aItemsArray{
    int i = 0; // 记录循环次数
    float menuWidth = 0.0;   // 记录菜单的宽度
    for (NSDictionary *lDic in aItemsArray) {
        NSString *vNormalImageStr = [lDic objectForKey:NOMALKEY];//正常时图片
        NSString *vHeligtImageStr = [lDic objectForKey:HEIGHTKEY];//高亮图片
        NSString *vTitleStr = [lDic objectForKey:TITLEKEY];// title
